...
np.random.seed(102)
step_size, batch_size, max_epochs = 0.14, 100, 100

# GLOBAL PARAMETERS FOR NETWORK ARCHITECTURE
number_of_layers = 5
width_of_layers = [400,200,200,100]  # only matters if number of layers > 1
...
def main():
  ...
  # Evaluate on Test
  ################################
  testset = net.forward(X_test)
  final = np.argmax(testset,axis=1)[:,np.newaxis]
  print(final.shape)
    
  test_out = np.concatenate((np.expand_dims(np.array(range(len(X_test)),dtype=int), axis=1), final), axis=1)
...
class LinearLayer:
  ...
  #################################################
  # Implementing Backward Pass for Linear
  #################################################
  def backward(self, grad):
    
    self.grad_weights = self.input.T @ grad
    self.grad_bias = np.sum(grad, axis=0)
    temp = grad @ self.weights.T

    #print("input x = ", self.input.shape)
    #print("weights = ", self.weights.shape)
    #print("bias = ", self.bias.shape)
    #print("z loss = ", grad.shape)
    #print("weight loss = ", self.grad_weights.shape)    
    #print("bias loss = ", self.grad_bias.shape)
    #print("output loss = ", temp.shape)
    #print("\n\n")
    #pause()
    return temp
...
class FeedForwardNeuralNetwork:
  ...
  def __init__(self, input_dim, output_dim, hidden_dim, num_layers, activation="ReLU"):
    ...
    if num_layers == 1:
      ...
      self.layers = [LinearLayer(input_dim, hidden_dim[0])]
      self.layers.append(Sigmoid() if activation=="Sigmoid" else ReLU())
      for i in range(num_layers-2):
        self.layers.append(LinearLayer(hidden_dim[i], hidden_dim[i+1]))
        self.layers.append(Sigmoid() if activation=="Sigmoid" else ReLU())
      self.layers.append(LinearLayer(hidden_dim[-1], output_dim))
...
#####################################################
# Utility Functions for Loading and Displaying
#####################################################
